...
document.querySelector("#salesmanInfo").innerHTML += `<h1>Sales Agent</h1>`

salesByWeek.forEach(sale => {
    let firstName = sale.sales_agent["first_name"];
    let lastName = sale.sales_agent["last_name"];
    document.querySelector("#salesmanInfo").innerHTML += `<hr/>`;
    document.querySelector("#salesmanInfo").innerHTML += `<h3>${firstName} ${lastName}</h3>`;
        for ( const currentVehicle of Object.entries(sale.vehicle)){
            document.querySelector("#salesmanInfo").innerHTML += `<div>${currentVehicle[0]} : ${currentVehicle[1]}</div>`
        }
        document.querySelector("#salesmanInfo").innerHTML += `<h4>Profit: $${sale.gross_profit}`
    })
